...
const Leaderboard = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const unparsedUserData = await AsyncStorage.getItem("userData");
      console.log("Fetched userData:", unparsedUserData);

      if (unparsedUserData) {
        const parsedData = JSON.parse(unparsedUserData);
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const times = response.data.workoutTime.map((time) => {
          const user = usersData.find((user) => user.user_id === time.user_id);
          return {
            ...time,
            username: user ? user.username : "Unknown",
          };
  ...
  useEffect(() => {
    const calculateTotalTimes = (times) => {
      ...
      times.forEach((time) => {
        if (time.username !== "Unknown") {
          ...
          timeMap[time.user_id].totalSeconds += timeStringToSeconds(
            time.workout_time
          );
        }
      });
    ...
    setCurrentTimeUserEntry(
      rankedTimes.find((entry) => entry.username === username)
    );
  ...
  /*useEffect(() => {
    const credentials = {
      awsAccessKey: Constants.expoConfig.extra.awsAccessKey,
      awsSecretKey: Constants.expoConfig.extra.awsSecretKey,
      awsRegion: Constants.expoConfig.extra.awsRegion,
      awsBucket: Constants.expoConfig.extra.awsBucket,
    };

    // Only update credentials if they exist in Constants
    if (
      ...
      console.error("Missing AWS credentials in expoConfig.");
    }
  }, []); */

  // Run AWS API calls only after credentials are loaded
  /*useEffect(() => {
    if (awsCredentials) {
      AWS.config.update({
      ...
      const getProfilePictureUrl = async () => {
        try {
          const response = await s3
            .listObjectsV2({
              Bucket: awsCredentials.awsBucket,
              Prefix: "profile_pics/",
            })
            .promise();
          ...
          response.Contents.forEach((item) => {
            const fileName = item.Key.split("/").pop(); // Extract filename
            const userId = fileName.replace("profilePic.png", ""); // Extract user_id
            urls[
              userId
            ] = `https://calisthenix.s3.ap-southeast-2.amazonaws.com/${item.Key}`;
          });
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={tw`flex-1`}>
          ...
          <View style={tw`mx-2 mb-6`}>
            <View style={tw`flex-row rounded-xl bg-zinc-800/50 p-1`}>
              <TouchableOpacity
                style={tw`flex-1 py-2 px-4 rounded-lg ${
                  selectedTab === "Tab1" ? "bg-orange-500" : ""
                }`}
                onPress={() => setSelectedTab("Tab1")}
              >
                <Text
                  style={tw`text-sm font-semibold text-center ${
                    selectedTab === "Tab1" ? "text-white" : "text-zinc-400"
                  }`}
                >
                  Workout Count
                </Text>
              ...
              <TouchableOpacity
                style={tw`flex-1 py-2 px-4 rounded-lg ${
                  selectedTab === "Tab2" ? "bg-orange-500" : ""
                }`}
                onPress={() => setSelectedTab("Tab2")}
              >
                <Text
                  style={tw`text-sm font-semibold text-center ${
                    selectedTab === "Tab2" ? "text-white" : "text-zinc-400"
                  }`}
                >
                  Workout Time
                </Text>
            ...
            >
              <View style={tw`flex-row items-center mb-4`}>
                <View style={tw`relative`}>
                  <Image
                    source={{
                      uri:
                        ...
                        profilePicUrls?.[currentUserEntry?.user_id]
                          ? profilePicUrls[currentUserEntry?.user_id]
                          : profilePicUrls?.blackDefaultProfilePic ||
                  ...
                  <View
                    style={tw`absolute -bottom-1 -right-1 bg-orange-500 rounded-full w-6 h-6 items-center justify-center`}
                  >
                    <Text style={tw`text-xs font-bold`}>
                      {selectedTab === "Tab1"
                        ? currentUserEntry?.rank || "-"
                        : currentTimeUserEntry?.rank || "-"}
                    </Text>
                  </View>
                </View>
                <View style={tw`ml-4`}>
                  ...
                  <View style={tw`flex-row items-center`}>
                    <Ionicons
                      ...
                      style={tw`mr-1`}
                    />
                    <Text style={tw`text-sm text-orange-300`}>
                      Your Monthly Rank
                    </Text>
                  </View>
                </View>
                  ...
                  <View style={tw`items-center w-1/3`}>
                    <View style={tw`relative`}>
                      <Image
                        source={{
                          uri:
                            rankedEntries[1]?.user_id &&
                            profilePicUrls?.[rankedEntries[1]?.user_id]
                              ? profilePicUrls[rankedEntries[1]?.user_id]
                              : `https://calisthenix.s3.ap-southeast-2.amazonaws.com/profile_pics/blackDefaultProfilePic.png`,
                        }}
                      ...
                      />
                      <View
                        style={tw`absolute -bottom-1 -right-1 bg-gray-300 rounded-full w-6 h-6 items-center justify-center`}
                      >
                        <Text style={tw`text-xs font-bold text-black`}>2</Text>
                      </View>
                    </View>
                    <Text
                      style={tw`text-sm font-semibold mt-2 text-center text-white`}
                      numberOfLines={1}
                    >
                      {selectedTab === "Tab1"
                  ...
                  <View style={tw`items-center w-1/3`}>
                    <View style={tw`relative`}>
                      <Image
                        source={{
                          uri:
                            rankedEntries[0]?.user_id &&
                            profilePicUrls?.[rankedEntries[0]?.user_id]
                              ? profilePicUrls[rankedEntries[0]?.user_id]
                              : `https://calisthenix.s3.ap-southeast-2.amazonaws.com/profile_pics/blackDefaultProfilePic.png`,
                        }}
                      ...
                      />
                      <View
                        style={tw`absolute -bottom-1 -right-1 bg-yellow-400 rounded-full w-7 h-7 items-center justify-center`}
                      >
                        <MaterialCommunityIcons
                          name="crown-outline"
                          size={14}
                  ...
                  <View style={tw`items-center w-1/3`}>
                    <View style={tw`relative`}>
                      <Image
                        source={{
                          uri:
                            rankedEntries[2]?.user_id &&
                            profilePicUrls?.[rankedEntries[2]?.user_id]
                              ? profilePicUrls[rankedEntries[2]?.user_id]
                              : `https://calisthenix.s3.ap-southeast-2.amazonaws.com/profile_pics/blackDefaultProfilePic.png`,
                        }}
                      ...
                      />
                      <View
                        style={tw`absolute -bottom-1 -right-1 bg-amber-600 rounded-full w-6 h-6 items-center justify-center`}
                      >
              ...
              <View style={tw`space-y-3`}>
                {(selectedTab === "Tab1" ? rankedEntries : rankedTimeEntries)
                  ...
                  .map((entry, index) => (
                    <View
                      key={entry.username}
                      style={tw`flex-row items-center p-3 bg-zinc-800 rounded-xl`}
                    >
